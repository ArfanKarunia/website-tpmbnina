...
import { X, Save, Stethoscope, Plus, Trash2, Search, Loader2, UserCheck, UserPlus, History, Baby, HeartPulse, User, AlertTriangle } from "lucide-react";
...
// --- INTERFACES ---
interface Patient { 
  id: string; name: string; address: string; birth_date: string; 
  nik?: string; hpht?: string; husband_name?: string; 
}
interface Medicine { id: string; name: string; price: number; stock: number; }
interface SelectedMedicine { id: string; name: string; qty: number; price: number; }
interface MedicalRecord {
  id: string; visit_date: string; patient_id: string; patient_name: string;
  diagnosis: string; action: string; therapy: string; staff_name: string;
  weight: number; blood_pressure: string; heart_rate: number; temperature: number; oxygen_saturation: number;
  service_fee: number; medicine_cost: number; total_price: number; risk_level: string;
}
interface ModalProps {
  isOpen: boolean; onClose: () => void; recordToEdit?: MedicalRecord | null;
}
...
export default function AddMedicalRecordModal({ isOpen, onClose, recordToEdit }: ModalProps) {
  ...
  const [ancData, setAncData] = useState({ husband_name: "", hpht: "", gravida: "", usg_type: "2D", leopold1: "", leopold2: "", leopold3: "", leopold4: "", djj: "" });
  ...
  const [medicineLoadError, setMedicineLoadError] = useState(false);
  ...
  // --- EFFECTS ---
  useEffect(() => { if(ancData.hpht) setCalcResult(calculatePregnancy(ancData.hpht)); }, [ancData.hpht]);

  useEffect(() => {
    if (isOpen) {
      const initData = async () => {
        // 1. Fetch Master Medicines
        ...
        if (recordToEdit) {
          // --- EDIT MODE ---
          setFormData({
              patient_id: recordToEdit.patient_id, visit_date: recordToEdit.visit_date,
              diagnosis: recordToEdit.diagnosis.replace(/G\d+\s-\s/, ''), 
              action: recordToEdit.action?.split('\n\n[ANC Data]')[0] || "",
              staff_name: recordToEdit.staff_name || "Bidan Nina",
              weight: recordToEdit.weight?.toString() || "", blood_pressure: recordToEdit.blood_pressure || "",
              heart_rate: recordToEdit.heart_rate?.toString() || "", temperature: recordToEdit.temperature?.toString() || "",
              spo: recordToEdit.oxygen_saturation?.toString() || "", service_fee: recordToEdit.service_fee,
              risk_level: recordToEdit.risk_level || "",
          });
          setPatientQuery(recordToEdit.patient_name || ""); 
          checkPatientHistory(recordToEdit.patient_id); 

          if(recordToEdit.diagnosis.toLowerCase().includes('hamil') || recordToEdit.diagnosis.toLowerCase().includes('anc') || recordToEdit.action.includes('[ANC Data]')) {
               setIsAncMode(true);
          }

          const { data: items } = await supabase.from('visit_items').select('*').eq('visit_id', recordToEdit.id);

          if (items && items.length > 0) {
               const recoveredCart = items.map((i: any) => {
                   const foundMed = medData?.find(m => m.id === i.item_id);
                   return {
                       id: i.item_id, name: foundMed ? foundMed.name : "Obat (Data Lama)", 
                       qty: i.qty, price: i.price_at_transaction 
                   };
               });
               setSelectedMedicines(recoveredCart);
               setOldMedicineTrack(recoveredCart.map(i => ({ name: i.name, qty: i.qty })));
               setMedicineLoadError(false);
          } else {
               setSelectedMedicines([]);
               setOldMedicineTrack([]);
               if (recordToEdit.therapy && recordToEdit.therapy.length > 3) setMedicineLoadError(true);
          }
        } else {
          // --- NEW MODE ---
          setFormData({
          ...
          setIsAncMode(false); setPatientQuery(""); setSelectedMedicines([]); setOldMedicineTrack([]); setVisitCount(0); setIsOldPatient(false); setMedicineLoadError(false);
        }
      };
      initData();
    }
  ...
  const checkPatientHistory = async (patientId: string) => {
     ...
     setVisitCount(count || 0); setIsOldPatient((count || 0) > 0);
  };

  const handleSearchPatient = (e: React.ChangeEvent<HTMLInputElement>) => {
    setPatientQuery(e.target.value); setShowPatientDropdown(true);
    if (e.target.value === "") { setFormData(prev => ({ ...prev, patient_id: "" })); setPatientResults([]); return; }
    if (searchTimeout.current) clearTimeout(searchTimeout.current);
    searchTimeout.current = setTimeout(async () => {
        setIsSearchingPatient(true);
        const { data } = await supabase.from("patients").select("*").ilike("name", `%${e.target.value}%`).limit(10); 
  ...
  const selectPatient = (patient: Patient) => {
      setFormData(prev => ({ ...prev, patient_id: patient.id })); setPatientQuery(patient.name); setShowPatientDropdown(false); checkPatientHistory(patient.id);
      setAncData(prev => ({ ...prev, husband_name: patient.husband_name || "", hpht: patient.hpht || "" }));
  };
  ...
  const addMedicineToCart = (medId: string) => {
    ...
    if (med) {
      setSelectedMedicines(prev => {
        ...
        return existing ? prev.map(m => m.id === medId ? {...m, qty: m.qty + 1} : m) : [...prev, { id: med.id, name: med.name, qty: 1, price: med.price }];
      });
      setMedicineLoadError(false);
    }
  };
  ...
  const updateQty = (index: number, newQty: number) => {
      ...
      setSelectedMedicines(prev => { const newCart = [...prev]; newCart[index].qty = newQty; return newCart; });
  };

  const removeMedicineFromCart = (index: number) => setSelectedMedicines(prev => prev.filter((_, i) => i !== index));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let finalTherapy = therapyString;
      if (selectedMedicines.length === 0 && recordToEdit && recordToEdit.therapy) {
           finalTherapy = recordToEdit.therapy; 
      }
      ...
      const payload = {
        ...
        staff_name: formData.staff_name, risk_level: formData.risk_level, 
        medicine_cost: medicineTotal, service_fee: formData.service_fee, total_price: grandTotal
      };
      ...
      if (recordToEdit) {
         ...
         if (oldItems && oldItems.length > 0) {
             for (const item of oldItems) {
                 // AMBIL STOK FRESH DARI DB (Jangan pakai state)
         ...
         // 3. SYNC TRANSAKSI (Hapus Lama)
         await supabase.from("transactions").delete().eq("visit_id", recordId);
      ...
      // --- 4. INSERT TRANSAKSI (Berlaku untuk Baru & Edit) ---
      const transactionsPayload = [];
      if (formData.service_fee > 0) {
          transactionsPayload.push({ 
            date: formData.visit_date, 
            description: `Jasa: ${selectedPatient?.name} (${isAncMode ? 'ANC' : 'Umum'})`, 
      ...
      if (medicineTotal > 0) {
          transactionsPayload.push({ 
            date: formData.visit_date, 
            description: `Obat: ${selectedPatient?.name}`, 
      ...
      // --- 5. INSERT VISIT ITEMS BARU & KURANGI STOK ---
      if (selectedMedicines.length > 0 && recordId) {
          ...
          // Kurangi Stok Baru (PAKAI DATA STOK FRESH LAGI BIAR AMAN)
          for (const item of selectedMedicines) {
              // Fetch ulang stok terkini dari DB (karena mungkin sudah berubah di langkah Revert tadi)
              const { data: freshMed } = await supabase.from('medicines').select('stock').eq('id', item.id).single();
              
              if (freshMed) {
                  await supabase.from("medicines").update({ stock: freshMed.stock - item.qty }).eq("id", item.id);
              }
          }
      }
  ...
  const handleSuccessClose = () => { setShowSuccess(false); onClose(); router.refresh(); };
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="relative">
                        <div className="flex justify-between items-center mb-1">
                            ...
                            {formData.patient_id && (<span className={`text-[10px] px-2 py-0.5 rounded-full font-bold flex items-center gap-1 ${isOldPatient ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}`}>{isOldPatient ? <UserCheck size={10}/> : <UserPlus size={10}/>} {isOldPatient ? `Pasien Lama (Ke-${visitCount})` : "Pasien Baru"}</span>)}
                        </div>
                        <div className="relative">
                  ...
                  <div className="border border-pink-200 bg-pink-50 rounded-xl p-4">
                      <div className="flex items-center gap-3 cursor-pointer" onClick={() => setIsAncMode(!isAncMode)}>
                          <div className={`w-5 h-5 rounded border flex items-center justify-center ${isAncMode ? 'bg-pink-500 border-pink-500' : 'bg-white border-gray-300'}`}>{isAncMode && <div className="w-2 h-2 bg-white rounded-full"></div>}</div>
                          <h4 className="text-sm font-bold text-pink-700 flex items-center gap-2"><Baby size={16}/> Pemeriksaan Kehamilan / ANC / USG</h4>
                      </div>
                      {isAncMode && (
                          <div className="mt-4 space-y-4 animate-in slide-in-from-top-2">
                              <div className="grid grid-cols-2 gap-4">
                                  <div><label className="text-xs font-bold text-gray-500 uppercase mb-1">HPHT (Penting!)</label><input type="date" className="w-full p-2 border rounded-xl focus:ring-2 focus:ring-pink-500" value={ancData.hpht} onChange={(e) => setAncData({...ancData, hpht: e.target.value})} />{calcResult.uk && (<p className="text-xs text-pink-600 mt-1 font-bold">UK: {calcResult.uk} • HPL: {calcResult.hpl}</p>)}</div>
                  ...
                  <div className="mt-4 p-4 bg-white border border-gray-200 rounded-xl shadow-sm">
                      <label className="block text-sm font-bold text-gray-700 mb-3 flex items-center gap-2">⚠️ Klasifikasi Status Resiko (KSPR) <span className="text-xs font-normal text-gray-400">(Pilih salah satu)</span></label>
                      <div className="flex flex-col sm:flex-row gap-4">
                        {RISK_LEVELS.map((level) => (
                           <label key={level.value} className={`flex-1 flex items-center p-3 border rounded-lg cursor-pointer transition-all ${formData.risk_level === level.value ? `bg-${level.color}-50 border-${level.color}-500 ring-1 ring-${level.color}-500` : 'hover:bg-gray-50 border-gray-200'}`}>
                              <input type="radio" name="risk_level" value={level.value} checked={formData.risk_level === level.value} onChange={(e) => setFormData(prev => ({ ...prev, risk_level: e.target.value }))} className={`w-4 h-4 text-${level.color}-600 focus:ring-${level.color}-500 border-gray-300`} />
                              <div className="ml-3"><span className="block text-sm font-bold text-gray-800">{level.label}</span><span className="block text-xs text-gray-500">{level.desc}</span></div>
                           </label>
                        ))}
                      </div>
                  </div>
                  ...
                  <div className="border border-blue-100 rounded-xl p-4">
                    <div className="flex justify-between items-center mb-2">
                        ...
                        {/* WARNING DATA LAMA */}
                        {medicineLoadError && recordToEdit?.therapy && (
                           <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 px-3 py-2 rounded-lg text-xs flex items-center gap-2 animate-in fade-in">
                              <AlertTriangle size={14} className="text-yellow-600"/>
                              <div><strong>Perhatian (Data Lama):</strong> Obat sebelumnya <i>"{recordToEdit.therapy}"</i> tersimpan sebagai teks biasa. <br/>Jika Anda tidak mengubah/menambah obat di bawah ini, teks lama tersebut <strong>tetap tersimpan</strong>.</div>
                           </div>
                        )}
                    </div>
                  ...
                  <div className="flex items-center gap-4 mt-4">
                    <div className="w-1/3"><label className="text-xs font-bold text-gray-500 uppercase mb-1 block">Petugas</label><div className="relative"><User size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"/><input type="text" required className="w-full pl-9 p-2 border rounded-xl text-sm focus:ring-2 focus:ring-blue-500 outline-none" value={formData.staff_name} onChange={(e) => setFormData({...formData, staff_name: e.target.value})} /></div></div>
                    <button type="submit" disabled={loading} className="flex-1 bg-blue-600 text-white font-bold py-2.5 rounded-xl hover:bg-blue-700 flex justify-center items-center gap-2 mt-5">{loading ? "Menyimpan..." : <><Save size={18} /> {recordToEdit ? "Simpan Perubahan" : "Simpan Data"}</>}</button>
                  </div>
                </form>
            </div>
          </div>
